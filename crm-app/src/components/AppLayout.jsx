...
function NavButton({ item, activeView, count, onSelect, compact = false }) {
  ...
  return (
    <button
      ...
      } ${selected ? 'border border-mint/30 bg-mint/10 text-mint shadow-[inset_3px_0_0_#C8A96A]' : item.id === 'metrics' ? 'border border-mint/15 bg-mint/[0.04] text-cream/90 hover:border-mint/30 hover:bg-mint/[0.08] hover:text-cream' : 'border border-transparent text-cream/90 hover:bg-slate-50 hover:text-cream'}`}
      type="button"
      onClick={() => onSelect(item.id)}
      aria-current={selected ? 'page' : undefined}
    >
      <Icon className={`h-4 w-4 ${selected || item.id === 'metrics' ? 'text-mint' : 'text-cream/65 group-hover:text-cream'}`} />
...
export default function AppLayout({ activeView, setActiveView, clinic, profile, isAdmin = false, navCounts = {}, onLogout, children }) {
  ...
  return (
    <div className="premium-grid min-h-screen bg-ink text-cream">
      <aside className="ui-dark-surface fixed inset-y-0 left-0 z-20 hidden h-screen w-72 flex-col overflow-hidden border-r border-slate-200 bg-[#0b0e15]/95 px-5 py-5 text-cream backdrop-blur-xl lg:flex">
        <div className="mb-6 shrink-0 border-l-2 border-mint pl-4">
          ...
          <p className="mt-1 truncate text-sm font-medium text-cream/75">Clínica · {clinic?.name || 'Anti-pérdida de pacientes'}</p>
        </div>
        ...
        <div className="mt-4 shrink-0 rounded-2xl border border-slate-200 bg-elevated/80 p-4 text-cream shadow-glow backdrop-blur">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-sm text-cream/65">{profile?.email}</p>
            </div>
          </div>
          <div className="mt-3 flex items-center justify-between border-t border-slate-200 pt-3">
            <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-cream/80">{isAdmin ? 'Owner / admin' : 'Recepción'}</span>
            <button className="rounded-lg p-2 text-cream/70 transition hover:bg-slate-50 hover:text-danger" type="button" onClick={onLogout} aria-label="Cerrar sesión">
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 px-4 py-3 shadow-[0_12px_40px_rgba(0,0,0,0.18)] backdrop-blur-xl md:px-8 lg:hidden">
          <div className="flex items-center justify-between gap-3">
            ...
            <button className="rounded-xl border border-slate-200 bg-white p-2.5 text-cream/75 transition hover:border-mint/30 hover:bg-slate-50 hover:text-cream" type="button" onClick={onLogout} aria-label="Cerrar sesión">
              <LogOut className="h-4 w-4" />
            </button>
          </div>
          <nav className="scrollbar-soft -mx-1 mt-3 flex snap-x gap-1 overflow-x-auto overscroll-x-contain pb-1" aria-label="Navegación principal móvil">
            {visibleNavItems.map((item) => (
