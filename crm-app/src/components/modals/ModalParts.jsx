...
export function ModalHeader({ title, subtitle, onClose, disabled }) {
  return (
    <div className="mb-5 flex items-start justify-between gap-4 border-b border-slate-200 pb-4">
      ...
      <button className="rounded-xl border border-slate-200 bg-card p-2 text-slate-500 transition hover:border-mint/30 hover:bg-elevated hover:text-cream disabled:cursor-not-allowed disabled:border-slate-200/60 disabled:text-slate-400" type="button" onClick={onClose} disabled={disabled} aria-label="Cerrar">
        <X className="h-4 w-4" />
      </button>
    </div>
  );
